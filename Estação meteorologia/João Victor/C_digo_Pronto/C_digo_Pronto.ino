#include <SPI.h> //cartão
#include <SD.h>  //cartão
#include <DHT.h>
#include <RTClib.h> //RTC relógio
...
void setup() {
  pinMode (7, OUTPUT); //led de sinalização
  ...
  while(!Serial); //para Leonardo/Micro/ Zero
    if(! rtc.begin()) {
      digitalWrite (7,HIGH); //Led piscado quando grava
      delay(100);
      digitalWrite(7,LOW);
      delay(100);
      digitalWrite (7,HIGH); //Led piscado quando grava
  ...
  if (myFile) {
    ...
    myFile.println("");
    myFile.println("Data;Hora;Temperatura;Umidade");
  }
  myFile.close();
}
